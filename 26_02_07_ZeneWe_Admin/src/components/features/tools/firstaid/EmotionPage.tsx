...
export function EmotionPage({ onComplete, onBack }: EmotionPageProps) {
  ...
  const [gateTriggerType, setGateTriggerType] = useState<'blocking' | 'soft'>('blocking');

  const handleSave = () => {
    // Check Guest Status
    if (status === 'guest') {
       setGateTriggerType('blocking');
      ...
      <div className="w-full h-full relative flex flex-col items-center justify-center z-10">
        ...
        */}
        <div className="relative z-10 w-full max-w-5xl px-4 md:px-8">
          <div
            className="backdrop-blur-xl bg-slate-900/60 rounded-[24px] px-4 py-6 shadow-2xl border border-white/10 flex flex-col gap-6 translate-y-[30px] max-w-[820px] mx-auto w-full"
            style={{
          ...
          >
            ...
            */}
            <div className="w-full overflow-x-auto py-2 -my-2 scrollbar-hide">
              <div className="min-w-[560px] grid grid-cols-8 gap-x-[2px] gap-y-3 justify-items-center">
                  ...
                  return (
                    <div key={`mood-${idx}`} className="flex flex-col items-center gap-2">
                       ...
                       <button
                         ...
                         className={`
                           w-[64px] h-[28px] rounded-full text-xs font-medium transition-all flex items-center justify-center
                           ${
                             isSelected
                               ? (isPos 
