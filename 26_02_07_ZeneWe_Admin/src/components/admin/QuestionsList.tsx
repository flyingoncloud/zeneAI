  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-6">
        {/* Table Header */}
        <div className="grid grid-cols-[40px_70px_1fr_90px_120px_80px_100px_120px] gap-2 px-4 py-2.5 text-xs text-slate-500 uppercase tracking-wider border-b border-white/[0.04] sticky top-0 bg-[#0F1115] z-10">
          <div></div>
          <div>ID</div>
          <div>题目</div>
          <div>模板</div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="divide-y divide-white/[0.03]">
            {filtered.map((q) => (
              <div
                key={q.id}
                className="grid grid-cols-[40px_70px_1fr_90px_120px_80px_100px_120px] gap-2 px-4 py-3.5 items-center group hover:bg-white/[0.02] transition-colors rounded-lg cursor-pointer"
              ...
              >
                ...
                <div className="min-w-0">
                  <p className="text-sm text-slate-200 truncate">{q.stem || q.internalTitle}</p>
                  <p className="text-[11px] text-slate-600 truncate mt-0.5">{q.internalTitle}</p>
                </div>
                ...
                <div>
                  {q.status === 'published' ? (
                    <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                      Published
                    </span>
                  ) : (
                    <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] bg-slate-500/10 text-slate-400 border border-slate-500/20">
                      Draft
                    </span>
                  )}
                </div>
                ...
                {/* Updated */}
                <div className="text-xs text-slate-500">{q.updatedAt}</div>
