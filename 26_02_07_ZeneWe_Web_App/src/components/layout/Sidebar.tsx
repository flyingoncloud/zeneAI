...
import { ClipboardList, ChevronLeft, ChevronDown, ChevronRight, Menu } from 'lucide-react';
...
import sloganImg from "figma:asset/70f510870d1344bdbbf375c4bbf6140102d2cc92.png";
...
const SidebarContent: React.FC<SidebarContentProps> = ({ isCollapsed, onClose, isMobile }) => {
  ...
  // State for First Aid expansion
  const [isFirstAidExpanded, setIsFirstAidExpanded] = useState(false);

  // Auto-expand if on sub-page
  useEffect(() => {
    if (['breathing', 'naming'].includes(currentView)) {
      setIsFirstAidExpanded(true);
    }
  }, [currentView]);
  ...
  const handleMenuClick = (item: any) => {
    ...
    // GUEST RESTRICTION
    if (['history', 'mood', 'naming', 'test'].includes(item.id)) {
        if (guardGuestAction(item.id)) return;
    }
  ...
  if (!isCollapsed) {
    return (
      <div className="flex flex-col h-full w-full">
         ...
         <div className="flex flex-col flex-shrink-0 pl-6 pr-5 py-6 relative">
            <div className="flex justify-between items-start">
               {/* Brand Content Wrapper - Scaled 120% */}
               <div className="flex flex-col origin-top-left scale-120">
                  ...
                  <img 
                    src={sloganImg} 
                    alt="遇见更好的自己"
                    className="mt-2 h-[25px] w-auto opacity-80 select-none pointer-events-none drop-shadow-[0_1px_2px_rgba(0,0,0,0.3)] object-contain object-left max-w-full"
                  />
               </div>

               {/* Only show collapse button on Desktop - Unscaled, Positioned Top Right */}
               {!isMobile && (
                 <Button 
                   variant="ghost" 
                   size="icon" 
                   onClick={toggleSidebar} 
                   className="text-slate-300 hover:text-white hover:bg-white/10 rounded-full w-8 h-8 mt-1 z-10"
                 >
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          <div className="flex-shrink-0">
            <div className="px-3 space-y-1.5">
              {CORE_ITEMS.map((item) => {
                // Active state: Direct match OR parent of active child
                const isDirectActive = item.id === currentView;
                const isChildActive = item.hasChildren && ['breathing', 'naming'].includes(currentView);
                ...
                return (
                  <div key={item.id} className="flex flex-col">
                    ...
                    >
                       ...
                       {item.hasChildren && (
                         <div className={`transition-transform duration-200 ${isFirstAidExpanded ? 'rotate-180' : ''}`}>
                            <ChevronDown size={14} className={isActive ? "text-violet-300" : "text-slate-400"} />
                         </div>
                       )}
                    </Button>
                              ...
                              {item.children.map(child => {
                                ...
                                return (
                                  <Button
                                    ...
                                    onClick={() => {
                                      // GUEST RESTRICTION
                                      if (['naming'].includes(child.id)) {
                                          if (guardGuestAction(child.id)) return;
                                      }

                                      setCurrentView(child.id as View);
                                      if(isMobile && onClose) onClose();
                                    }}
                                    className={`w-full justify-start h-9 pl-12 pr-4 text-[15px] font-medium transition-colors ${
                                      isChildItemActive
                                        ? 'text-violet-100 bg-white/5'
                                        : 'text-slate-300 hover:text-white hover:bg-white/5'
                                    }`}
                                  >
                                    <div className={`w-1.5 h-1.5 rounded-full mr-3 ${isChildItemActive ? 'bg-violet-400 shadow-[0_0_5px_rgba(139,92,246,0.8)]' : 'bg-transparent'}`} />
                                    {child.label}
                                  </Button>
                                )
                              })}
          ...
          <div className={cn("px-7 flex items-center justify-between mb-3 h-5 flex-shrink-0", !isMobile && "mt-10")}>
            <h3 className="text-xs font-semibold text-slate-400/80 uppercase tracking-wider">{t.common.recentChats}</h3>
          </div>

          {/* B. Recent Chats List */}
          <div 
            className="px-3 space-y-1 max-h-[192px] overflow-y-auto [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-white/20"
          >
                ...
                <Button
                  ...
                  className={`w-full justify-start text-left h-auto py-3 px-4 mb-1 border transition-all rounded-xl group ${
                    isActive 
                      ...
                      : 'text-slate-300 hover:text-slate-100 hover:bg-white/5 border-transparent'
                  }`}
                >
                  <div className="w-full overflow-hidden">
                    <div className="flex justify-between items-center mb-0.5">
                      <span className={`font-medium truncate text-sm ${isActive ? 'text-violet-200 group-hover:text-violet-100' : 'text-slate-200 group-hover:text-white'}`}>
                        {session.title || t.menu.chat}
                      </span>
                    </div>
                    <div className="text-xs text-slate-400 truncate opacity-70 group-hover:text-slate-300 transition-colors">
  ...
  // Render Collapsed Content (Desktop Only)
  return (
    <div className="flex flex-col h-full w-full">
      ...
      <div className="flex-1 overflow-hidden">
        <div className="flex flex-col items-center gap-6 mt-4 animate-in fade-in duration-300">
          <TooltipProvider delayDuration={200}>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button 
                  ...
                  size="icon" 
                  onClick={toggleSidebar} 
                  className="text-slate-300 hover:text-white hover:bg-white/10 rounded-full w-10 h-10 flex items-center justify-center transition-colors"
                >
            ...
            {(() => {
              const item = CORE_ITEMS[0]; // First Aid
              const isChildActive = ['breathing', 'naming'].includes(currentView);
              const isActive = currentView === 'first-aid' || isChildActive;
                ...
                <HoverCard openDelay={0} closeDelay={200}>
                  <HoverCardTrigger asChild>
                    <Button 
                      variant="ghost" 
                      size="icon" 
                      onClick={() => setIsFirstAidExpanded(prev => !prev)} /* Optional click behavior */
                    ...
                    >
                       <SafeIcon icon={Icons.Heart} size={22} className={isActive ? "fill-violet-500/20" : ""} />
                    </Button>
                  </HoverCardTrigger>
                  <HoverCardContent 
                    side="right" 
      ...
      <>
        {!isSidebarOpen && !['test', 'report-detail'].includes(currentView) && (
           <Button 
             ...
             size="icon" 
             onClick={toggleSidebar} 
             className="fixed top-6 left-6 z-50 text-slate-300 hover:text-white hover:bg-slate-900/40 rounded-full w-10 h-10 flex items-center justify-center transition-all duration-300 backdrop-blur-sm border border-white/5 shadow-sm"
           >
             <SafeIcon icon={Icons.Menu} size={22} />
           </Button>
        )}
        ...
        <Sheet open={isSidebarOpen} onOpenChange={toggleSidebar}>
          <SheetContent side="left" className="p-0 border-r border-white/10 bg-slate-900/95 backdrop-blur-xl w-[85%] max-w-[360px] text-slate-200 shadow-2xl">
